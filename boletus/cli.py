"""Boletus CLI — init, run, validate, agents, tasks."""
...
SCAFFOLD_CREW_YAML = '''# Boletus — Your AI Company
# Starts with CEO, CTO, CMO. They hire more agents as needed.
# Docs: https://github.com/Majny/boletus
...
def cmd_init(args):
    ...
    if not env_vars.get("SLACK_BOT_TOKEN"):
        ...
        print("     (find it in the boletus repo root)")
    ...
    if not env_vars.get("GITHUB_TOKEN"):
        ...
        print("  3. Name: boletus, Expiration: pick what suits you")
    ...
    with open(env_path, "w") as f:
        f.write("# Boletus — generated by boletus init\n")
...
def cmd_setup(args):
    ...
    def on_complete(config_path, business_description=""):
        ...
        bot = BoletusBot(config_path=config_path)
...
def cmd_run(args):
    ...
    try:
        from .bot import BoletusBot
        bot = BoletusBot(config_path=args.config)
        bot.start()
    except FileNotFoundError:
        print("No crew.yaml found. Starting setup wizard...")
        print("(You can also run: boletus init  for manual YAML setup)")
        print()
...
def cmd_doctor(args):
    ...
    checks.append((bool(config_path), f"crew.yaml: {config_path or 'NOT FOUND'}", "Run: boletus init"))
    ...
    print()
    if all_ok:
        print("All checks passed! Run: boletus run")
    else:
        print("Some checks failed. Fix the issues above, then run: boletus doctor")
...
def main():
    parser = argparse.ArgumentParser(
        prog="boletus",
        description="Boletus — Your first AI company",
    )
    parser.add_argument("--version", action="version", version=f"boletus {__version__}")
